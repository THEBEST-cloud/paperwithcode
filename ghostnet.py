...
class SqueezeExcite(nn.Module):
    def __init__(self, in_chs, se_ratio=0.25, reduced_base_chs=None,
        ...
        self.conv_reduce = nn.Conv2d(in_chs, reduced_chs, 1, bias=True) # 压缩通道
        self.act1 = act_layer(inplace=True)
        self.conv_expand = nn.Conv2d(reduced_chs, in_chs, 1, bias=True) # 扩张到原来通道

    def forward(self, x):              #[B,C,H,W]
        x_se = self.avg_pool(x)        #[B,C,1,1]
        x_se = self.conv_reduce(x_se)  #[B,reduced_chs,1,1]
        x_se = self.act1(x_se)         #[B,reduced_chs,1,1]
        x_se = self.conv_expand(x_se)  #[B,C,1,1]
        x = x * self.gate_fn(x_se)     #[B,C,H,W]
        return x    
...
if __name__=='__main__':
    ...
    model.eval()
    # print(model)
    input = torch.randn(32,3,640,640)
    y = model(input)
    print(y.size())