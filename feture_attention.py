...
class ScaleChannelAttention(nn.Module):
    ...
    def forward(self, x):
        global_x = self.avgpool(x)  # [N,in_C,H,W]---------->[N,in_C,1,1]
        global_x = self.fc1(global_x)  # [N,in_C,1,1]---------->[N,out_C,1,1]
        global_x = F.relu(self.bn(global_x))
        global_x = self.fc2(global_x) # [N,out_C,1,1]---------->[N,num_features,1,1]
        global_x = F.softmax(global_x, 1)
        return global_x # [N,num_features,1,1]
