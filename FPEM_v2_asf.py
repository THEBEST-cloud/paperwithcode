...
import math
import torch
from .feture_attention import ScaleFeatureSelection
...
class FPEM_v2_ASF(nn.Module):
    def __init__(self, in_channels, out_channels, inner_channels=256, attention_type='scale_channel_spatial'):
        super(FPEM_v2_ASF, self).__init__()
    ...
    def forward(self, x):
        ...
        f1,f2,f3,f4 = self._upsample_cat(f1,f2,f3,f4)
        # print(f1.shape, f2.shape, f3.shape, f4.shape)
        x = torch.cat((f1,f2,f3,f4),dim=1)
        fuse = self.concat_attention(x, [f1, f2, f3, f4])
        return fuse
...
if __name__ == "__main__":
    ...
    x.append(f3)
    x.append(f4)
    ffpm = FPEM_v2_ASF(in_channels=[32,64,128,256],out_channels=256)
