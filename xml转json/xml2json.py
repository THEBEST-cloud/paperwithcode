...
INITIAL_BBOXIds = 1
# PREDEF_CLASSE = {}
PREDEF_CLASSE = {'holothurian': 0, 'echinus': 1,
                 'scallop': 2, 'starfish': 3}
...
def convert(xml_paths, out_json):
    ...
    for image_id, xml_f in enumerate(xml_paths):
        ...
        # print(type(xml_f))
        filename = xml_f.split("D:/水下感知赛/光学任务/guang_forUser_A/forUser_A/val/box")[1]
...
if __name__ == '__main__':
    xml_path = r'D:/水下感知赛/光学任务/guang_forUser_A/forUser_A/val/box'  # 改一下读取xml文件位置
    xml_file = glob.glob(os.path.join(xml_path, '*.xml'))
    print(xml_file)
    convert(xml_file, r'D:\水下感知赛\光学任务\guang_forUser_A\forUser_A\val/guang_train.json')  # 这里是生成的json保存位置，改一下
