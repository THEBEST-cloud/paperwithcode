...
class MobileViT(nn.Module):
    def __init__(self,image_size,dims,channels,num_classes,depths=[2,4,3],expansion=4,kernel_size=3,patch_size=2):
        ...
        self.Conv_C1 = conv_bn(64,32,kernel_size=1)
        self.Conv_C2 = conv_bn(128,64,kernel_size=1)
    ...
    def forward(self,x):
        out = []
        y=self.conv1(x) #
        # print("第一层普通卷积：", y.shape)
        y=self.mv2[0](y)
        # print("第一层MobileBlock：", y.shape)
        y=self.mv2[1](y) #
        # print("第二层MobileBlock：", y.shape)
        y=self.mv2[2](y)
        # print("第三层MobileBlock：", y.shape)
        y=self.mv2[3](y) # 该层结果需要
        out1 =  self.Conv_C1(y)
        out.append(out1)
        # print("第四层MobileBlock：", y.shape)
        y=self.mv2[4](y) #
        # print("第五层MobileBlock：", y.shape)
        y=self.m_vits[0](y) # 该层结果需要 
        out2 =  self.Conv_C2(y)
        out.append(out2)
        # print("第一层MVitBlock：", y.shape)
        y=self.mv2[5](y) #
        # print("第六层MobileBlock：", y.shape)
        y=self.m_vits[1](y) # 该层结果需要 
        out.append(y)
        # print("第二层MVitBlock：", y.shape)
        y=self.mv2[6](y) #
        # print("第七层MobileBlock：", y.shape)
        y=self.m_vits[2](y)  
        # print("第三层MVitBlock：", y.shape)
        y=self.conv2(y) # 该层结果需要
        out.append(y)
        # print("第二层普通卷积：", y.shape)
...
def mobilevit_xxs():
    ...
    return MobileViT(640,dims,channels,num_classes=1000)
